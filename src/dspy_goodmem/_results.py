...
class RetrievalOutcome:
    ...
    def warning_text(self) -> str:
        ...
        if not self.statuses:
            return ""
        parts = [f"{s.code}: {s.message}" if s.message else s.code for s in self.statuses]
    ...
    *,
    ...
    for hit, memory_id in pending:
        ...
        if mem:
            ...
            hit.content_type = str(mem.get("contentType") or mem.get("content_type") or "")
        ...
        try:
            ...
            outcome.statuses.append(
                RetrievalStatus(
                    ...
                    message=("The retrieval stream ended badly; results may be " f"incomplete: {exc}"),
                )
            )
            outcome.partial = True
            return
